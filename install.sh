...
ark() { "$HOME/core-token/packages/core/bin/run" "$@" --token="myn"; }
echo 'myn() { "$HOME/core-token/packages/core/bin/run" "$@" --token="myn"; }' >> "$HOME/.bashrc"

rm -rf "$HOME/core-token"
git clone "https://github.com/Highjhacker/my_chain_bis_core" "$HOME/core-token" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/Highjhacker/my_chain_bis_core'"
    exit 1
fi
...
rm -rf "$HOME/.config/@myn"
rm -rf "$HOME/.config/@MyChainBis"
rm -rf "$HOME/.config/myn-core"
